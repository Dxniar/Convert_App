...
class DocxToExcelApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Docx -> Excel конвертер v2.0")
        width = 800
        height = 700
            ...
            for docx_path in self.docx_paths:
                ...
                for line in lines:
                    ...
                    if line_lower.startswith(ANSWER_KEYWORDS):
                        correct_raw = line.split(":")[1].strip().lower()
                        correct_letters = re.findall(r"[a-z]", correct_raw)
                    elif self.starts_with_letter_bracket(line):
                        answers.append(line)
                    else:
                        if answers:
